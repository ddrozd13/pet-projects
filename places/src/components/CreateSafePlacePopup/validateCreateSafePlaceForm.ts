...
export interface IValidateCreateSafePlaceFormData {
  address: string;
  description: string;
  imageSrc: string;
  capacity: number;
...
const validateCreateSafePlaceForm = (data: IValidateCreateSafePlaceFormData): IValidateCreateSafePlaceFormResult => {
  ...
  if(data.imageSrc.length === 0) {
    errors.imageSrc = 'Image Src must be have 0 characters';
  };
  if(data.imageSrc.length > 0 && !HTTPS_REGEX.test(data.imageSrc)) {
    errors.imageSrc = 'Image Src must be a valid URL';
  };
  if(data.description.length < 50) {
    errors.description = `Description must be less 50 characters`;
  };
  if(data.capacity <= 0) {
