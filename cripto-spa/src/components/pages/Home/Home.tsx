            ...
            <table cellSpacing="0">
              ...
              <tbody>
                <tr className={styles.table_header}>
                  <th>Coin</th>
                  <th>Price</th>
                  <th className={styles.table_body_item}>24h changes</th>
                  <th className={styles.table_body_item}>Market Cap</th>
                  <th></th>
                </tr>
                {coins && coins.slice((currentPage - 1) * perPage, (currentPage * perPage)).map((coin) => {
                  return (
                    <tr key={coin.id} className={styles.table_body} onClick={() => navigate(`/coin/${coin.id}`)}>
                      ...
                      <td
                        className={clsx(Math.sign(coin.changePercent24Hr) === -1 || -0 ? styles.red : styles.green, styles.table_body_item)}
                      >
                        {signMath(coin.changePercent24Hr)}
                        {round(coin.changePercent24Hr, 3)}
                        %
                      </td>
                      <td className={styles.table_body_item}>$ {numberWithCommas(round(Number(coin.marketCapUsd.toString().slice(0, -6))))}</td>
